...
void TofTriggerCalib::FullCalib(int refSlX,int refSlY, bool verboseMode)
{
  ...
  for (int j = 0; j < 4; j++) {
    dataTree->SetBranchStatus(Form("adc%d", Station*4 + j), 1);
    dataTree->SetBranchStatus(Form("t%d", Station*4 + j), 1);
  ...
  // first do referecne slab calibrations
  ...
  for(int i=0;i<nSlabs;i++)
  {
    ...
    // get already stored trigger delayes for currently calibrated slab
    // trigger delay first gotten when calibrating reference slabs
    if( tr_delays[i][refSlY] ) t0_trSlX = tr_delays[i][refSlY];
    if( tr_delays[refSlX][i] ) t0_trSlY = tr_delays[refSlX][i];

    // run trigger calibrations for slabs which are not reference
    // slabs, pass on trigger delay
...
bool TofTriggerCalib::TriggerCalib(int Plane, int Slab, double refTime, int refSl, bool verboseMode)
{
  ...
  // get PMT0 histogram for this slab and this pixel (w.r.t. reference slab)
  ...
  if(ref_h0->GetEntries()>minEntries)
  {
    ...
    // // dealing with bad channel S1P0S4P0
    // if (Station == 1 && Plane == 0 && Slab == 4)
    //   // tag this slab's PMT bad
    //   pixel0_Sl_mean_0 = -99.;
  ...
  // iterrate over the rest of the slabs in the ref plane
  for(int i=0;i<nSlabs;i++)
  {
    ...
    if(pixel_h0->GetEntries()>minEntries)
    {
      ...
      //VP: FIXME: this should also blacklist broken channel!
      //thispixel_Sl_mean_0 = pixel_h0->GetMean();
      // if (Station == 1 && Plane = 0 && Slab == 4)
      // 	thispixel_Sl_mean_0 = -99.;
    ...
    {
      // one of the PMTs does not have time info (insufficient number of entries?)
    ...
    {
      // if (pixel0_Sl_mean_0 == -99.)
      // 	// do not include this slab's PMT time into the delay!
      // 	Tr_delay = (pixel0_Sl_mean_1 - thispixel_Sl_mean_1) ;
      // else
      ...
      if(Plane==0)
      {
        if( tr_delays[Slab][i] )
        {
	  // previously calculated trigger delay for this pixel
	  // (possibly arrived from the other direction)
          double t1 = tr_delays[Slab][i];
	  // Sum trigger delay w.r.t. central pixel + delay of that
	  // pixel
          double t2 = Tr_delay + refTime;
	  // average the two calculated delays (for reference slabs,
	  // averaging identical number (refTime == t1))
          double trD = (t1 + t2)/2.;
          if(fabs(t1-t2)>30. && verboseMode)
          {
	    // the two calculated trigger delays should not differ by more than 30 ps
      ...
      if(!refTime)
      {
	// no reference time ... calibrating reference slabs. save
	// times of PMTs from the other reference slab into T0 file - T0 for those slabs
...
void TofTriggerCalib::FillHistograms()
{
  ...
  while(i<n_entries)
  {
    ...
    if(_slX!=-99)
    {
      // correct ADC for T1P0S6P0
      // is this a badly behaving channel?
      if (Station == 1 && _slX == 6 && _adc0 > 3800 && _adc1 > 1400 && _adc1 < 2600)
