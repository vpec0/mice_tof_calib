...
int main(int argc, char **argv) {
  ...
  if (twCalib) {
    cout<<"Doing TW calibration."<<endl;
    TWcalib = new TofTWCalib(dataTree);
    doTWCalib();
  } else if (triggerCalib || t0Calib) {
    cout<<"Reading in  TW calibration."<<endl;
  ...
  if (triggerCalib) {
    cout<<"Doing Trigger calibration."<<endl;
    TofTrigger = new TofTriggerCalib(trStation, TWcalib, dataTree);
    doTriggerCalib();
  } else if (t0Calib) {
    cout<<"Reading in Trigger calibration."<<endl;
  ...
  if (t0Calib) {
    cout<<"Doing T0 calibration."<<endl;
    // arg 1 = station
  ...
  cout<<"main(): Done with calibration."<<endl;
...
void doTWCalib() {
  ...
  tw->cd();
  TWcalib->FullCalib(0,tof0_sXmax,tof0_sYmax,verboseMode,false); // mod
  TWcalib->FullCalib(1,tof1_sXmax,tof1_sYmax,verboseMode,false);
