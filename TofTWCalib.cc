...
void myFcn(Int_t &npar, Double_t *grad , Double_t &fval, Double_t *p, Int_t iflag  )
{
  ...
  for (int i = 0; i <n; ++i ) {
    x[0] = coords[i];
    // original: ignores bins with 0 error (1 event)
    if(errors[i]){
      tmp = ( values[i] - fitf(x,p))/errors[i];
      chi2 += tmp*tmp;
    }// new attempt: sets uncertainty to [something] if 1 event
    else{
      tmp = ( values[i] - fitf(x,p))/300; // seems to be 175 at high ADC counts, but goes up to ~600 at low values
      chi2 += tmp*tmp;
    }
  }
  fval = chi2;
...
TofTWCalib::TofTWCalib(string n)
{
  ...
  nPar = 4;
  adcCut = 0.1;
  f1 = new TF1("f1",fitf,200.,6000.,nPar);
  f1->SetLineWidth(1);
  f1->SetLineColor(2);
...
void TofTWCalib::FullCalib(int st, int refSlA, int refSlB, bool verboseMode, bool Minuit)
{
  ...
  // set on only subset of branches
  dataTree->SetBranchStatus("*",0);
  for (int j = 0; j < 2; j++) {
    TString label("slabA");
    label[4] += st*2 + j;
    dataTree->SetBranchStatus(label, 1);
    // cout<<"Turning On branch "<<label<<endl;
  }
  for (int j = 0; j < 4; j++) {
    dataTree->SetBranchStatus(Form("adc%d", st*4 + j), 1);
    dataTree->SetBranchStatus(Form("t%d", st*4 + j), 1);
    // cout<<"Turning On branch "<<Form("adc%d", st*4 + j)<<endl;
    // cout<<"Turning On branch "<<Form("t%d", st*4 + j)<<endl;
  }


  this->ReferenceSlabCalib(st, refSlA, refSlB, verboseMode, Minuit);
...
{
  // prepare histograms for each channel of the reference slabs
  //
  // charge hist
  vector<TH1F*> h1d;
  for(int i=0;i<4;i++)
  {
    ...
    h1d.push_back( new TH1F(name.str().c_str(), name.str().c_str(), 300, 0, 6000) );
  }
  ...
  for(int i=0;i<4;i++)
  {
    ...
    h2d.push_back( new TH2F(name.str().c_str(), name.str().c_str(), 120, 0, 6000, 4000,-50000,50000) );
  }
  ...
  while(i<n_entries)
  {
    ...
    {
      // cout<<_slX<<" "<<_slY<<" "
      // 	  <<_adc0<<" "<<_adc1<<" "<<_adc2<<" "<<_adc3
      // 	  <<" "<<_t0<<" "<<_t1<<" "<<_t2<<" "<<_t3<<endl;
	...
	// This will fill in difference in an average tdc of the 2
	// slabs. No charge preselection is made for a "reference"
	// slab. This is heavilly influenced by TW.
      }
    }
  }
  tof0.Draw();

  // find "means" in the ADC acquired distributions
  double adc_mean[4], adc_rms[4];
  for (int i = 0; i < 4; i++) {
    adc_mean[i] = h1d[i]->GetBinCenter( h1d[i]->GetMaximumBin() );
    adc_rms[i] = h1d[i]->GetRMS();
    //cout<<"ReferenceSlabCalib "<<h1d[0]->GetTitle()<<" "<<adc_mean[0]<<" +-"<<adc_rms[0]<<endl;
  }


  // pick a referrence PMT in each slab - based on charge - higher mean charge better!
  ...
  // where is the "mean" in DT distribution
  double mean_tof = tof0.GetBinCenter( tof0.GetMaximumBin() );

  // DT hists constrained to the area of the "mean"
  TH1F tof1("tof1","tof1", 200, mean_tof-2500, mean_tof+2500);
  TH1F tof2("tof2","tof2", 200, mean_tof-2500, mean_tof+2500);


  // loop over the entries again
  // get 2D plots, only from within a charge cut region of the ref PMT
  // relative distance from the channel's "mean" is compared to adcCut
  // (10% by default)
  //
  // creates DT vs charge hists, each channel w.r.t. reference channel
  // in the other slab. Done only for events from the reference pixel.
  i = 0;
  int goodEntries=0;
  while(i<n_entries)
  {
    ...
    i++;
    if(slabX == _slX && slabY == _slY)
      // looking at the right, reference, slabs
    {
  ...
  // do not calibrate this station if there's not enough stats even in the reference pixel
  if(goodEntries<500)
  {
  ...
  // create profiles for each channel, and fit them
  // profiles are ...
  for(unsigned int i=0;i<4;i++)
  {
    ...
    if (!func) {
	cerr<<"TofTWCalib::ReferenceSlabCalib(): fit did not perform well. Will try to redo."<<endl;
	if (Minuit)
	    cerr<<"    This time with ROOT fit."<<endl;
	func = RootFit(hpf[i], verboseMode);
    }
  ...
  c1_tw->SetName(Form("RefPixFit_tof%d",station));
  c1_tw->SetTitle(Form("RefPixFit TOF%d",station));
  c1_tw->Update();
  c1_tw->Write();
  ...
  c1_tw->SetName(Form("RefPixResol_tof%d",station));
  c1_tw->SetTitle(Form("RefPixResol TOF%d", station));
  c1_tw->Write();
...
{
  cout<<"TOF"<<Station<<"  Plane "<<Plane<<"  Slab "<<Slab<<endl;

  // retrieve DT vs PMT Q hists for this PMT
  ...
  // TW hists must have a minimum nuber of events!
  ...
  // fit both PMTs, using MINUIT and a simple ROOT fit
  ...
  // compare results of the 2 fit methods, save the output calib constants
  CompareFits(Station, Plane, Slab, refslab);

  // do the partial output - only things related to this channel, the
  // calib constants are not saved here, they are saved in the
  // CompareFits() call
  cout<<"   Entrie`s = "<< events <<" -- calibrated"<<endl;
  TH2F* h1, *h2;
  c1_tw->Clear();
  c1_tw->Divide(2,2);
  c1_tw->cd(1);
...
{
  double adc_mean = h2->GetXaxis()->GetBinCenter( h2->ProjectionX()->GetMaximumBin() );
  ...
  for(int st=0;st<3;st++)
  {
    ...
    for(int pl=0;pl<2;pl++)
    {
      ...
      for(int sl=0;sl<10;sl++)
      {
        ...
        TH2F *Ph2d_0 = new TH2F(name0.str().c_str(), name0.str().c_str(), 120, 0, 6000, 10000,-50000,50000);
        hists[st][pl][sl].push_back( Ph2d_0 );

        stringstream name1;
        name1<<_name<<"h2d"<<name.str()<<"Pmt1";
        TH2F *Ph2d_1 = new TH2F(name1.str().c_str(), name1.str().c_str(), 120, 0, 6000, 10000,-50000,50000);
...
void TofTWCalib::FillHistograms(int st, int refSlX, int refSlY)
{
  ...
  while(i<n_entries)
  {
    ...
    // is this a badly behaving channel?
    if (st == 1 && _slX == 6 && _adc0 > 3800 && _adc1 > 1400 && _adc1 < 2600)
      // need to correct this slab's charge
      _adc0 -= 2000;


    // is event with ref slab hit? are the slabs defined in this event?
    if(refSlX==_slX && _slX != -99 && _slY != -99)
    {
      // fill in raw DT between this slab (plane 1, _slY) and the reference refSlX
      raw_t[st][1][_slY]->Fill( double(_t1+_t0)/2. - double(_t3+_t2)/2. );

      // correct reference slab times for TW (from ::ReferenceSlabCalib())
      ...
      {
	// fill in the DT w.r.t. the corrected reference time!
	t1[st][1][_slY]->Fill( double(_T0+_T1)/2. - double(_t3+_t2)/2. ); // avg slab DT
	hists[st][1][_slY][0]->Fill(_adc2, double(_T0+_T1)/2. - double(_t2));
	hists[st][1][_slY][1]->Fill(_adc3, double(_T0+_T1)/2. - double(_t3));
      }
    }
    // do the same for X-plane (reference slab from Y-plane)
...
void TofTWCalib::MinuitFit(int Station, int Plane, int Slab, int Pmt, bool verboseMode)
{
  ...
  // vvvv This will be done anyway. Slab TWs are always fit with MINUIT and ROOT fit
  // if (!func) {
  //   // did not work, try ROOT fit
  //   cout<<"TofTWCalib::MinuitFit(): Redoing fit using ROOT fit."<<endl;
  //   func = RootFit(pf, verboseMode);
  //   if (!func) {
  //     cerr<<"TofTWCalib::MinuitFit(): Something went terribly wrong,"
  // 	  <<" can't even get RootFit to work."<<endl;
  //   }
  // }

  // normalize the fit's parameters -- possibly because the TW
  // histos are first shifted to cross 0 at ADC mean?
  if (!func)
      // the fit failed, but we still want to get the function's parameters
      func = pf->GetFunction("f1_minuit");
...
TF1* TofTWCalib::MinuitFit(TProfile* pfh, double adc_mean, double adc_rms, bool verboseMode)
{
  ...
  values.resize(0);
  errors.resize(0);
  for(int bin=pfh->FindBin(200); bin<nBinsX;bin++)
  {
  ...
  double P1 = pfh->GetYmax();
  static double vstart[4] = {500., P1 , -1e6 , -1e8};
  static double step[4] = {0.1 , 0.1 , 0.01 , 0.001};
  minuit->mnparm(0, "a1", vstart[0], step[0], 0, 1e6, ierflg);
  minuit->mnparm(1, "a2", vstart[1], step[1], 0, 0, ierflg);
  minuit->mnparm(2, "a3", vstart[2], step[2], -1e10, 0, ierflg);
  minuit->mnparm(3, "a4", vstart[3], step[3], -1e15, 0, ierflg);
  // minimize
  ...
  TF1 *func = new TF1(*f1);
  func->SetLineColor(4);
  func->SetName( Form("%s_minuit", func->GetName()) );
  pfh->GetListOfFunctions()->Add(func);
  //cout<<"Minuit Fit  chi2/ndf = "<<chi2/ndf<<endl;

  if (ierflg != 0) {
      // something went wrong
      std::cerr<<"TofTWCalib::MinuitFit(): "
	       <<"MINUIT fit would not execute normally: "<<ierflg<<". "
	       <<pfh->GetName()<<std::endl;
      return 0;
  }
...
{
  f1->SetParameters(500. , pfh->GetYmax(), -1e6, -1e8);
  f1->SetParLimits(0, -500, 500);
  f1->SetParLimits(2, -1e10, 0);
  f1->SetParLimits(3, -1e15, 0);
  f1->SetLineColor(2);

  if(verboseMode) pfh->Fit("f1","RBW");
  else  pfh->Fit("f1","RQBW");
  pfh->GetFunction("f1")->SetName("f1_rootfit");
  pfh->GetFunction("f1_rootfit")->SetRange(-f1->GetParameter(0) + 10, f1->GetXmax());
...
void TofTWCalib::CompareFits(int Station, int Plane, int Slab, int refslab)
{
  ...
  // compare the resulting RMS of slab DT after the correction
  // use the better behaving results and save the constants!
  ...
  if(sigma<sigma_m)
  {
    cout<<"TofTWCalib::CompareFits: Choosing ROOT fit results."<<endl;
  ...
  {
    cout<<"TofTWCalib::CompareFits: Choosing MINUIT fit results."<<endl;
...
TProfile* TofTWCalib::MakeProfile(TH2F* h2)
{
  if(h2->GetEntries()<5e3)
      // rebin charge axis if low statistics
      h2->RebinX();

  double dt_mean = h2->GetMean(2); // limit axis range to only around
																	 // mean (not maximum this time)
																	 // both, profile and plotting
																	 // +- 350 bins, 25 ps/bin => +-~9000 ps
																	 // not very effective
  ...
  // new: gaussian fit in slices
  TProfile* hpf = new TProfile( (name+"_pfh").c_str(), "adc", h2->GetNbinsX(), 0, 6000);
  for(int i=0; i < h2->GetNbinsX(); i++){
    TH1D *slice = h2->ProjectionY("tmp", i, i);

    int nonempty = 0;
    for(int j=maxbin-350; j<=maxbin+350; j++){
      if(slice->GetBinContent(j)>0) nonempty++;
      if(nonempty > 4) break;
    }
    float rms = slice->GetRMS();
    float entries = slice->GetEntries();

    if(nonempty == 0) continue;

    /* old */
    /* else if(nonempty < 1){ */
    // this is still suspicious, as 2 bins close to each other,
    // still with low statistics, would give unreasonably low error
    // estimate
    /*   */
    else if(nonempty < 5){
      // require at least 5 non-empty bins so that the Gausian fit has meaning
      if (entries < 20)
	continue; // require at least 20 entries to properly estimate
		  // the RMS and the Mean (arbitrarily picked
		  // constrain)

      float content = slice->GetMean();

      float error = sqrt( pow(content,2) + pow(300, 2) ); // sets error to 300 because TProfile is silly
      if (rms > 300)
	  error = sqrt( pow(content,2) + pow(rms, 2) ); // sets error to 300 because TProfile is silly

      hpf->SetBinEntries(i, 1);
      hpf->SetBinContent(i, content);
      hpf->SetBinError(i, error);
    }else{
      TFitResultPtr result = slice->Fit("gaus", "L S Q", "", dt_mean-3500, dt_mean+3500);

      float content = result->Parameter(1);
      float error = sqrt( pow(content,2) + pow(result->Error(1), 2) ); // because TProfile is silly
      hpf->SetBinEntries(i, 1);
      hpf->SetBinContent(i, content);
      hpf->SetBinError(i, error);
    }
  ...
  hpf->SetMarkerStyle(kFullCircle);
  hpf->SetMarkerSize(0.4);
  hpf->SetLineColor(kBlack);
  hpf->SetLineWidth(2);
  return hpf;
}
