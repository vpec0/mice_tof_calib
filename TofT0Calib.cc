...
void TofT0Calib::FullCalib(int refSlA, int refSlB, bool verboseMode)
{
  ...
  int triggerStation = TriggerCalib->GetStation();
  for (int j = 0; j < 2; j++) {
    TString label("slabA");
    label[4] += triggerStation*2 + j;
    dataTree->SetBranchStatus(label, 1);
    // cout<<"Turning On branch "<<label<<endl;
  }
...
void TofT0Calib::PlaneCalib(int plane, int refSl, bool verboseMode)
{
  ...
  // loop over all slabs in the plane
  for(int i=0;i<nSlabs;i++)
  {
    _pmtPar.slab = i;
    int Slab_A, Slab_B;
    // define horizontal (A) and vertical (B) slabs
    ...
    // k is an ordinal representattion of 4 PMTs in the pixel; 1 and 2
    // in plane 0, 3 and 4 in plane 1
    int k;
    if(!plane) k = 0;
    else k = 2;
    int max = k+2;
    ...
    while(k<max)
    {
      ...
      // set histogram's range arounf it's mean
	...
	// fit T distro with a gaussian, around 1n the estimated
	// position of electron peak (1st found in TOF2, last found in
	// TOF0)
	...
	// retrieve mean of the electron (hopefully) peak
        t0 = fgaus.GetParameter("Mean");
        //cout <<"plane "<<plane<<"   slab "<<i<<"  pmt "<<k<<"  t0 = "<<t0<<" - "<<dT<<endl;
	// save T0 parameter, ToF at speed of light subtracted.
        _pmtPar.par[0] = t0 - dT; // dT is initialized when L supplied via ::SetL()
        _pmtPar.par[1] = refSl;
        channelParTree->Fill();
